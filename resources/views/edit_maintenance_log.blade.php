...
<div class="container-fluid nss_style add-service-date">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">
            ...
            @else
                <span class="rental insert_dates"><a href="{{ route('maintenance-log.index') }}">Maintenance Log</a></span>
            @endif
            >>
            <span class="insert_dates">Edit Entry</span>
        </h1>
    </div>
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('log-update', $log->id) }}">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                <select id="unit_id" name="unit_id" class="form-control @error('unit_id') is-invalid @enderror" required>
                                    ...
                                    <option value="{{ $unit->id }}" {{ $log->unit_id == $unit->id ? 'selected' : '' }}>{{ $unit->name }}</option>
                                    @endforeach
                                </select>
                                @error('unit_id')
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                <input type="date" id="date" name="date" class="form-control @error('date') is-invalid @enderror" value="{{ old('date', $log->date) }}" required onkeydown="return false;">
                                @error('date')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="row mb-3">
                            <label for="description" class="col-md-4 col-form-label text-md-end">
                                {{ $log->type === 'issue' ? __('Issue Description') : __('Maintenance Description') }}
                                <span class="text-danger">*</span>
                            </label>
                            <div class="col-md-6">
                                <textarea id="description" name="description" rows="4" class="form-control @error('description') is-invalid @enderror" required>{{ old('description', $log->description) }}</textarea>
                                @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                <input type="number" id="amount" name="amount" step="0.01" min="0" class="form-control @error('amount') is-invalid @enderror" value="{{ old('amount', $log->amount) }}" required>
                                @error('amount')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                <textarea id="notes" name="notes" rows="3" class="form-control @error('notes') is-invalid @enderror">{{ old('notes', $log->notes) }}</textarea>
                                @error('notes')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6 offset-md-4">
                                ...
                                @else
                                    <a href="{{ route('maintenance-log.index') }}" class="btn btn-secondary ms-2">{{ __('Cancel') }}</a>
                                @endif
                            </div>
                        </div>

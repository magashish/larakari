    ...
    <div class="container-fluid nss_style">
        ...
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">
                <span class="rental insert_dates">Issue Items</span>
            </h1>
        </div>
            ...
            <div class="col-xl-12 col-md-12 mb-4">
                <div class="row mb-3">
                    <div class="col-md-4 offset-md-4 col-form-label text-md-end">
                        <label for="unit_type" class="col-form-label text-md-end">{{ __('Select The Unit # to View Issues') }}</label>
                        <select id="unit_type" class="form-control">
                            <option value="{{ route('issue-items.index') }}">All Units</option>
                            @foreach(unit_type_array() as $unit)
                            <option value="{{ route('issue-items.by_unit', $unit->id) }}" {{ $unit_id == $unit->id ? 'selected' : '' }}>{{ $unit->name }}</option>
                            @endforeach
                        </select>
                        @if ($unit_id)
                        <span class="all_dates"><a href="{{ route('issue-items.index') }}">View All Units</a></span>
                        @endif
                    </div>
                </div>
            </div>
        ...
        <div class="d-sm-flex align-items justify-content-end mr-2 mb-4">
            <div><a href="{{ route('issue-items.create') }}" class="btn btn-primary">Add Issue Item</a></div>
        </div>
